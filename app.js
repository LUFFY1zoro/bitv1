const STATIC_ONLY_MODE = true;
const STATIC_PREFIX = "socialPilotStatic:";

const nativeFetch = window.fetch.bind(window);
window.fetch = async (input, options = {}) => {
  const url = typeof input === "string" ? input : input.url;
  if (typeof url === "string" && url.startsWith("/api/")) {
    return handleStaticApi(url, options);
  }
  return nativeFetch(input, options);
};

function staticRead(key, fallback) {
  try {
    return JSON.parse(localStorage.getItem(STATIC_PREFIX + key) || JSON.stringify(fallback));
  } catch {
    return fallback;
  }
}

function staticWrite(key, value) {
  localStorage.setItem(STATIC_PREFIX + key, JSON.stringify(value));
  return value;
}

function staticJson(payload, status = 200) {
...
async function staticPayload(options) {
...
function staticContentPack(topic = "your brand update", platform = "instagram", tone = "friendly") {
  const clean = topic.trim() || "your brand update";
  const tag = clean.replace(/[^a-z0-9]/gi, "").slice(0, 24) || "Content";
  return {
    caption: `Hook: ${clean}\n\nHere is a ${tone} ${platform} post idea built to be clear, useful, and easy to engage with. Save this if it helps, and comment what you would try next.`,
    hashtags: [`#${tag}`, "#SocialMedia", "#ContentCreator", "#Growth", "#StudentProject"],
    cta: "Comment your favorite idea or save this for later.",
    image_ideas: [
      `Close-up visual showing ${clean}`,
      "Before-and-after carousel with simple labels",
      "Behind-the-scenes photo with a short text overlay",
    ],
    platform_tip: `${platform}: lead with the strongest visual or first sentence, then ask one clear question.`,
    tone,
  };
}

function staticAccountStatus(email = "") {
  const accounts = staticRead("social_accounts", {});
  const ig = accounts[(email || "local-user").toLowerCase()]?.instagram || {};
  const ready = Boolean(ig.instagram_user_id && ig.instagram_access_token);
  return [
    {
      platform: "Instagram (your account)",
      handle: ig.handle || "not connected",
      status: ready ? "Ready for official publishing on server version" : "Add Graph token and user ID for server publishing",
    },
    { platform: "X/Twitter", handle: "not connected", status: "Server connector needed" },
    { platform: "TikTok", handle: "not connected", status: "Future OAuth connector" },
    { platform: "LinkedIn", handle: "not connected", status: "Future OAuth connector" },
    { platform: "YouTube", handle: "not connected", status: "Future OAuth connector" },
  ];
}

async function handleStaticApi(url, options = {}) {
  const parsed = new URL(url, "http://static.local");
  const path = parsed.pathname;
  const method = (options.method || "GET").toUpperCase();
  const payload = method === "GET" ? {} : await staticPayload(options);

  if (method === "GET") {
    if (path === "/api/tasks") return staticJson(staticRead("tasks", []));
    if (path === "/api/schedule") return staticJson(staticRead("schedule", []));
    if (path === "/api/accounts") return staticJson(staticAccountStatus(parsed.searchParams.get("email") || ""));
    if (path === "/api/social-account") {
      const key = (parsed.searchParams.get("email") || "local-user").toLowerCase();
      const account = staticRead("social_accounts", {})[key]?.instagram || {};
      return staticJson({
        ok: true,
        platform: "instagram",
        handle: account.handle || "",
        instagram_user_id: account.instagram_user_id || "",
        has_token: Boolean(account.instagram_access_token),
        token_preview: account.instagram_access_token ? `${account.instagram_access_token.slice(0, 4)}...${account.instagram_access_token.slice(-4)}` : "",
        updated_at: account.updated_at || "",
      });
    }
    if (path === "/api/team") return staticJson(staticRead("team", [{ name: "Project Lead", role: "Owner" }]));
    if (path === "/api/drafts") return staticJson(staticRead("drafts", []));
    if (path === "/api/templates") return staticJson(staticTemplates());
    if (path === "/api/brand-kit") return staticJson(staticRead("brand_kit", { company: "SocialPilot AI", voice: "helpful", colors: "#0f766e, #2563eb", logo: "SP" }));
    if (path === "/api/notifications") return staticJson(staticRead("notifications", [{ title: "Static mode", body: "This version stores data in your browser.", read: false }]));
    if (path === "/api/activity") return staticJson(staticRead("activity", []));
    if (path === "/api/analytics") {
      const schedule = staticRead("schedule", []);
      const history = staticRead("post_history", []);
      const tasks = staticRead("tasks", []);
      return staticJson({
        kpis: {
          scheduled_posts: schedule.filter(item => !item.done).length,
          publish_attempts: history.length,
          successful_posts: history.filter(item => item.posted).length,
          open_tasks: tasks.filter(item => !item.done).length,
        },
        accounts: staticAccountStatus(""),
        history: history.slice(-12),
        summary: { engagement_note: "Static mode stores local demo activity only. Live analytics require the Python/server version and official APIs." },
      });
    }
  }
  ...
  if (method === "POST") {
    if (path === "/api/ask") {
      return staticJson({ ok: true, answer: `Static AI helper:\n\n${payload.prompt || "Ask a question"}\n\nSuggested next step: break the task into 3 small actions, finish the first one now, then save your result as a draft.` });
    }
    if (path === "/api/draft") {
      return staticJson({ ok: true, text: staticContentPack(payload.topic, payload.platform, payload.tone).caption });
    }
    if (path === "/api/content-pack") return staticJson({ ok: true, pack: staticContentPack(payload.topic, payload.platform, payload.tone) });
    if (path === "/api/strategy") {
      return staticJson({
        ok: true,
        strategy: {
          goal: payload.goal || "grow engagement",
          recommendation: "Post consistently, reuse winning topics, and keep every post focused on one clear action.",
          content_pillars: ["Education", "Proof", "Community", "Behind the scenes"],
          weekly_plan: ["Monday: tip post", "Wednesday: behind-the-scenes", "Friday: CTA or offer"],
        },
      });
    }
    if (path === "/api/reply") return staticJson({ ok: true, reply: "Thanks for asking. Yes, we can help with that. Send us a message and we will share the details." });
    if (path === "/api/image-prompt") return staticJson({ ok: true, prompt: `Create a polished ${payload.platform || "instagram"} visual for: ${payload.idea || "a social post"}. Use strong lighting, readable text space, and a modern brand style.` });
    if (path === "/api/competitor") return staticJson({ ok: true, analysis: `Static competitor note for ${payload.handle || "@competitor"}: review posting frequency, content themes, hooks, and comment quality using public information only.` });
    if (path === "/api/media/analyze") return staticJson({ ok: true, analysis: "Static media analysis: describe the main subject, add a strong hook, include 5-8 hashtags, and end with a question for comments." });
    if (path === "/api/post") {
      const history = staticRead("post_history", []);
      const row = {
        ...
        message: payload.dry_run === false ? "Static mode saved the publish attempt. Real posting needs the Python/server API." : "Dry run only. Nothing was posted.",
        created_at: staticNow(),
      };
      history.push(row);
      staticWrite("post_history", history);
      return staticJson(row);
    }
    if (path === "/api/tasks") {
      const tasks = staticRead("tasks", []);
      const task = { id: Math.max(0, ...tasks.map(item => item.id || 0)) + 1, text: payload.text || "", done: false };
      tasks.push(task);
      staticWrite("tasks", tasks);
      return staticJson(task);
    }
    if (path.startsWith("/api/tasks/") && path.endsWith("/done")) {
      const id = Number(path.split("/")[3]);
      const tasks = staticRead("tasks", []).map(item => item.id === id ? { ...item, done: true } : item);
      staticWrite("tasks", tasks);
      return staticJson(tasks.find(item => item.id === id) || {});
    }
    if (path === "/api/schedule") {
      const schedule = staticRead("schedule", []);
      const item = { id: Math.max(0, ...schedule.map(row => row.id || 0)) + 1, kind: payload.kind || "post", when: payload.when || "", text: payload.text || "", platform: payload.platform || "", done: false };
      schedule.push(item);
      staticWrite("schedule", schedule);
      return staticJson(item);
    }
    if (path === "/api/schedule/run") return staticJson({ ok: true, message: "Static mode checked due items. Real background publishing needs a hosted server." });
    if (path === "/api/drafts") {
      const drafts = staticRead("drafts", []);
      const draft = { id: Math.max(0, ...drafts.map(item => item.id || 0)) + 1, title: payload.title || "Untitled Draft", platform: payload.platform || "instagram", content: payload.content || "", created_at: staticNow() };
      drafts.push(draft);
      staticWrite("drafts", drafts);
      return staticJson({ ok: true, draft });
    }
    if (path === "/api/team") return staticJson({ ok: true, team: staticWrite("team", payload.team || []) });
    if (path === "/api/brand-kit") return staticJson({ ok: true, brand_kit: staticWrite("brand_kit", payload) });
    if (path === "/api/settings") return staticJson({ ok: true, message: "Static mode cannot save server secrets. Use hosting environment variables for deployment." });
    if (path === "/api/social-account") {
      const email = (payload.email || "local-user").toLowerCase();
      const accounts = staticRead("social_accounts", {});
      accounts[email] = accounts[email] || {};
      accounts[email].instagram = {
        handle: payload.handle || "",
        instagram_user_id: payload.instagram_user_id || "",
        instagram_access_token: payload.instagram_access_token || accounts[email].instagram?.instagram_access_token || "",
        updated_at: staticNow(),
      };
      staticWrite("social_accounts", accounts);
      return staticJson({
        ok: true,
        message: "Instagram account saved in this browser. Real publishing needs server mode.",
        account: {
          handle: accounts[email].instagram.handle,
          instagram_user_id: accounts[email].instagram.instagram_user_id,
          token_preview: accounts[email].instagram.instagram_access_token ? "set" : "not set",
        },
      });
    }
  }

  return staticJson({ ok: false, message: "Static endpoint not found." }, 404);
}
...
async function loginUser(event) {
  ...
  if (!client) {
    if (!email) {
      document.getElementById("loginMessage").textContent = "Email is required.";
      return;
    }
    const users = staticRead("users", {});
    const saved = users[email.toLowerCase()];
    if (!saved && password) {
      document.getElementById("loginMessage").textContent = "No static account found. Use Create Account first.";
      return;
    }
    localStorage.setItem("socialPilotUser", JSON.stringify(saved || { name, email, provider: "static" }));
    showApp();
...
async function signupUser() {
  ...
  if (!client) {
    ...
    users[key] = { ...profile, name, email, provider: "static" };
    staticWrite("users", users);
    localStorage.setItem("socialPilotUser", JSON.stringify(users[key]));
    showApp();
    applyAdminVisibility();
    showToast("Static account created.", "success");
    return;
  }
...
async function continueWithGoogle() {
  ...
  if (!client) {
    ...
    localStorage.setItem("socialPilotUser", JSON.stringify(user));
    showApp();
    applyAdminVisibility();
    showToast("Demo Google login started locally.", "success");
    return;
  }
...
async function continueWithInstagram() {
  ...
  if (!client) {
    const user = {
      ...
      email: "instagram-demo@example.com",
      avatar_url: "",
      provider: "static-instagram",
    };
    localStorage.setItem("socialPilotUser", JSON.stringify(user));
    showApp();
    applyAdminVisibility();
    showToast("Demo Instagram login started locally.", "success");
    return;
  }
  document.getElementById("loginMessage").textContent =
    "Instagram OAuth needs your Meta app callback configured in Supabase. If it is enabled, the redirect will start now.";
  const { error } = await client.auth.signInWithOAuth({
    provider: "instagram",
    options: {
      redirectTo: window.location.origin,
  ...
  if (error) {
    document.getElementById("loginMessage").textContent =
      `${error.message}. If Supabase does not show Instagram as a provider, use the Accounts page token connection until Meta OAuth is approved.`;
  }
}
...
async function publishPost() {
  ...
  try {
    ...
    showToast(data.posted ? "Post published successfully." : "Publish check completed.", data.posted ? "success" : "info");
    loadAnalytics();
  } catch (error) {
...
async function saveCurrentDraft() {
  ...
  if (!content.trim()) {
    ...
    showToast("Write or generate content first.", "error");
    return;
  }
  try {
...
async function saveInstagramAccount() {
  ...
  if (!user.email) {
    ...
    showToast("Login first before saving Instagram.", "error");
    return;
  }
  ...
  try {
    ...
    loadAnalytics();
    loadActivity();
    showToast("Instagram account saved.", "success");
  } catch (error) {
    output.textContent = error.message;
    showToast(error.message, "error");
  }
